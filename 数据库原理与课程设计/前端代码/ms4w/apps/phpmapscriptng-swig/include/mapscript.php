...
abstract class mapscript {
	...
	const MS_TOKEN_BINDING_DOUBLE = 370;
	...
	static function msLoadMapFromString($buffer,$new_mappath) {
		$r=msLoadMapFromString($buffer,$new_mappath);
		if (is_resource($r)) {
...
class webObj {
	...
	function __get($var) {
		...
		if ($var === 'extent') return new rectObj(webObj_extent_get($this->_cPtr));
...
class styleObj {
	...
	function __get($var) {
		...
		if ($var === 'backgroundcolor') return new colorObj(styleObj_backgroundcolor_get($this->_cPtr));
...
class labelObj {
	...
	function __get($var) {
		if ($var === 'leader') return new labelLeaderObj(labelObj_leader_get($this->_cPtr));
...
class classObj {
	...
	function getMetaData($name) {
		return classObj_getMetaData($this->_cPtr,$name);
	}

	function setMetaData($name,$value) {
		return classObj_setMetaData($this->_cPtr,$name,$value);
	}

	function getFirstMetaDataKey() {
		return classObj_getFirstMetaDataKey($this->_cPtr);
	}

	function getNextMetaDataKey($lastkey) {
		return classObj_getNextMetaDataKey($this->_cPtr,$lastkey);
	}
...
class labelCacheObj {
	...
	function __get($var) {
		if ($var === 'slots') return new labelCacheSlotObj(labelCacheObj_slots_get($this->_cPtr));
		$func = 'labelCacheObj_'.$var.'_get';
		if (function_exists($func)) return call_user_func($func,$this->_cPtr);
		if ($var === 'thisown') return swig_mapscript_get_newobject($this->_cPtr);
		return $this->_pData[$var];
	}
...
class resultCacheObj {
	...
	function __set($var,$value) {
		if ($var === 'usegetshape') return resultCacheObj_usegetshape_set($this->_cPtr,$value);
		if ($var === 'thisown') return swig_mapscript_alter_newobject($this->_cPtr,$value);
		$this->_pData[$var] = $value;
	}
	...
	function __get($var) {
		...
		if ($var === 'usegetshape') return resultCacheObj_usegetshape_get($this->_cPtr);
...
class layerObj {
	...
	function getMetaData($name) {
		return layerObj_getMetaData($this->_cPtr,$name);
	}

	function setMetaData($name,$value) {
		return layerObj_setMetaData($this->_cPtr,$name,$value);
	}

	function removeMetaData($name) {
		return layerObj_removeMetaData($this->_cPtr,$name);
	}

	function getFirstMetaDataKey() {
		return layerObj_getFirstMetaDataKey($this->_cPtr);
	}

	function getNextMetaDataKey($lastkey) {
		return layerObj_getNextMetaDataKey($this->_cPtr,$lastkey);
	}
	...
	function executeWFSGetFeature($layer) {
		return layerObj_executeWFSGetFeature($this->_cPtr,$layer);
	}
	...
	function setProcessing($directive) {
		layerObj_setProcessing($this->_cPtr,$directive);
	}
...
class mapObj {
	...
	function __construct($filename="") {
		...
		$this->_cPtr=new_mapObj($filename);
	}
	...
	function drawLegend() {
		$r=mapObj_drawLegend($this->_cPtr);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new imageObj($r);
	...
	function getMetaData($name) {
		return mapObj_getMetaData($this->_cPtr,$name);
	}

	function setMetaData($name,$value) {
		return mapObj_setMetaData($this->_cPtr,$name,$value);
	}

	function removeMetaData($name) {
		return mapObj_removeMetaData($this->_cPtr,$name);
	}

	function getFirstMetaDataKey() {
		return mapObj_getFirstMetaDataKey($this->_cPtr);
	}

	function getNextMetaDataKey($lastkey) {
		return mapObj_getNextMetaDataKey($this->_cPtr,$lastkey);
	}
...
class rectObj {
	...
	function project($projin,$projout) {
		return rectObj_project($this->_cPtr,$projin,$projout);
	}
...
class pointObj {
	...
	function __set($var,$value) {
		if ($var === 'x') return pointObj_x_set($this->_cPtr,$value);
		if ($var === 'y') return pointObj_y_set($this->_cPtr,$value);
		if ($var === 'thisown') return swig_mapscript_alter_newobject($this->_cPtr,$value);
		$this->_pData[$var] = $value;
	}

	function __get($var) {
		if ($var === 'x') return pointObj_x_get($this->_cPtr);
		if ($var === 'y') return pointObj_y_get($this->_cPtr);
		if ($var === 'thisown') return swig_mapscript_get_newobject($this->_cPtr);
		return $this->_pData[$var];
	}
	...
	function project($projin,$projout) {
		return pointObj_project($this->_cPtr,$projin,$projout);
	}
...
class lineObj {
	...
	function project($projin,$projout) {
		return lineObj_project($this->_cPtr,$projin,$projout);
	}
...
class shapeObj {
	...
	function project($projin,$projout) {
		return shapeObj_project($this->_cPtr,$projin,$projout);
	}
